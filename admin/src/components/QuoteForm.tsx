...
export default function QuoteForm({ quote, projects = [], onClose, onSuccess }: QuoteFormProps) {
  ...
  const [projectTitle, setProjectTitle] = useState(quote?.projectTitle || '')
  const [projectId, setProjectId] = useState<number | null>(quote?.projectId ?? null)
  ...
  const [taxRate, setTaxRate] = useState(quote?.taxRate ?? 20)
  const [vatEnabled, setVatEnabled] = useState(quote?.vatEnabled ?? true)
  const [discount, setDiscount] = useState<Discount>(
    quote?.discount || { enabled: false, type: 'friends-family', percent: 10, amount: 0 }
  )
  const [paymentMethod, setPaymentMethod] = useState(quote?.paymentMethod || 'bank-transfer')
  const [paymentTermsType, setPaymentTermsType] = useState(quote?.paymentTermsType || 'net-30')
  const [paymentTermsLabel, setPaymentTermsLabel] = useState(quote?.paymentTermsLabel || 'Net 30')
  const [paymentSchedule, setPaymentSchedule] = useState<PaymentScheduleItem[]>(
    quote?.paymentSchedule || getDefaultSchedule('net-30', 0)
  )
  ...
  const handleTermsChange = (value: string) => {
    ...
    setPaymentSchedule(getDefaultSchedule(value, total))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(quote?.id ? { id: quote.id, ...body } : body),
      })
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-white border border-[#b0b0b0]/30 p-6 rounded-lg">
        ...
        <div className="space-y-6">
          ...
          {paymentTermsType && paymentTermsType !== 'custom' && (
            <PaymentSchedule
