  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <label className="inline-flex items-center gap-2 px-5 py-3 bg-[#2a1a0a] text-white rounded-xl font-semibold text-sm uppercase tracking-widest cursor-pointer hover:bg-[#2a4f66] transition-colors">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <input
          ...
          className="flex-1 px-4 py-3 bg-white rounded-xl border border-[#e8d4b8]/30 text-sm focus:outline-none focus:border-[#2a1a0a] text-white"
        />
        <select
          value={filter}
          onChange={(e) => setFilter(e.target.value)}
          className="px-4 py-3 bg-white rounded-xl border border-[#e8d4b8]/30 text-sm focus:outline-none focus:border-[#2a1a0a] text-white"
        >
          {types.map((t) => (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-12 text-center border border-[#e8d4b8]/20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {filtered.map((asset) => (
            <div
              key={asset.id}
              className="group bg-white rounded-2xl overflow-hidden border border-[#e8d4b8]/20 hover:border-[#2a1a0a] transition-shadow"
            >
              <div className="aspect-square bg-[#0A0A0A] flex items-center justify-center relative">
                ...
                <div className="absolute inset-0 bg-[#2a1a0a]/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <a
                    href={asset.path}
                    target="_blank"
                ...
                {asset.usedIn && asset.usedIn.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {asset.usedIn.slice(0, 2).map((ref: string, i: number) => (
                      <span key={i} className="text-[10px] px-1.5 py-0.5 bg-[#2a1a0a] text-white rounded font-semibold uppercase tracking-wider">
                        {ref}
                      </span>
                    ))}
