...
export default function AboutEditor() {
  ...
  if (loading) return <div className="p-8 text-[#91715c]">Loading...</div>;
    ...
    <div className="max-w-5xl">
      ...
      <div className="mb-12">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-[clamp(36px,5.5vw,64px)] font-black text-[#2a1a0a] tracking-[-2px] uppercase leading-[0.95]">About Page Editor</h1>
            <p className="text-sm text-[#b89a6e] mt-4 font-semibold uppercase tracking-[0.5px]">Edit headline, bio, quote, images and press pack</p>
          </div>
          <div className="flex items-center gap-3">
            <a href="/about" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-7 py-3 border-2 border-[#2a1a0a] rounded-full text-[13px] font-semibold uppercase tracking-[1.5px] text-[#2a1a0a] hover:bg-[#FAFAF7] transition">
              View on Site
            </a>
            <button
              onClick={toggleVisibility}
              disabled={!section}
              className="inline-flex items-center gap-2 px-5 py-3 border-2 border-[#91715c]/30 rounded-full text-[13px] font-semibold uppercase tracking-[1.5px] text-[#91715c] hover:bg-[#91715c] hover:text-white transition disabled:opacity-50"
            >
      ...
      <div className="bg-white border border-[#91715c]/30 rounded-xl p-6 md:p-8">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#FAFAF7]">
          ...
          <p className="text-xs font-semibold text-[#91715c] uppercase tracking-[3px]">About / Quote</p>
            ...
            <div key={field.key}>
              ...
              {field.type === 'textarea' ? (
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-2.5 bg-white border border-[#91715c]/30 rounded-lg text-[#2a1a0a] text-sm focus:outline-none focus:border-[#2a1a0a] transition-colors resize-y"
                />
              ) : field.type === 'image' ? (
                <div>
                  {content[field.key] ? (
                ...
                <input
                  ...
                  onChange={(e) => updateContent(field.key, e.target.value)}
                  placeholder={field.label}
                  className="w-full px-4 py-2.5 bg-white border border-[#91715c]/30 rounded-lg text-[#2a1a0a] text-sm focus:outline-none focus:border-[#2a1a0a] transition-colors"
                />
              )}
            </div>
          ))}
        ...
        <div className="mt-8 flex items-center gap-4">
          <button
            onClick={saveSection}
            disabled={saving}
            className="px-7 py-3 border-2 border-[#2a1a0a] rounded-full text-[13px] font-semibold uppercase tracking-[1.5px] text-[#2a1a0a] hover:bg-[#2a1a0a] hover:text-white transition disabled:opacity-50"
          >
