...
const PIPELINE_STYLES: Record<string, { bg: string; text: string; border: string }> = {
  inquiry: { bg: 'bg-[#8a8a8a]', text: 'text-[#a0a0a0]', border: 'border-[#b0b0b0]/30' },
  quoted: { bg: 'bg-[#7a7a7a]/10', text: 'text-[#7a7a7a]', border: 'border-[#7a7a7a]/30' },
  approved: { bg: 'bg-[#91715c]/10', text: 'text-[#91715c]', border: 'border-[#91715c]/30' },
  'in-progress': { bg: 'bg-[#7a7a7a]/10', text: 'text-[#7a7a7a]', border: 'border-[#7a7a7a]/30' },
  completed: { bg: 'bg-[#91715c]/10', text: 'text-[#91715c]', border: 'border-[#91715c]/30' },
  invoiced: { bg: 'bg-[#7a7a7a]/10', text: 'text-[#7a7a7a]', border: 'border-[#7a7a7a]/30' },
...
const inputClass = 'w-full px-4 py-2.5 bg-white border border-[#b0b0b0]/30 rounded-lg text-[#7a7a7a] placeholder-[#b0b0b0] text-sm focus:outline-none focus:border-[#7a7a7a] transition-all'
const cardClass = 'bg-white border border-[#b0b0b0]/30 p-6'
...
export default function ProjectsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-12">
        <p className="text-xs text-[#b0b0b0] tracking-[3px] uppercase font-semibold mb-4">Project Management</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        <div className="bg-white border border-[#b0b0b0]/30 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="p-2 bg-[#7a7a7a] text-white rounded-lg"><FolderOpen size={16} /></div>
          </div>
          <p className="text-[clamp(28px,4vw,42px)] font-black text-[#111] leading-none tracking-[-1px]">{loading ? '–' : stats.total}</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">Total Projects</p>
        </div>
        <div className="bg-white border border-[#b0b0b0]/30 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="p-2 bg-[#7a7a7a] text-white rounded-lg"><Clock size={16} /></div>
          </div>
          <p className="text-[clamp(28px,4vw,42px)] font-black text-[#7a7a7a] leading-none tracking-[-1px]">{loading ? '–' : stats.inquiry}</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">Inquiries</p>
        </div>
        <div className="bg-white border border-[#b0b0b0]/30 p-5">
          <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">In Progress</p>
        </div>
        <div className="bg-white border border-[#b0b0b0]/30 p-5">
          <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">Pipeline Value</p>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={saveProject} className={`${cardClass} mb-8 space-y-5`}>
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-[#b0b0b0]/30">
            <p className="text-xs text-[#b0b0b0] tracking-[3px] uppercase font-semibold">New Project</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-[#b0b0b0] uppercase tracking-[3px] mb-2">Type</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-[#b0b0b0] uppercase tracking-[3px] mb-2">Event Date</label>
              <input type="date" value={form.eventDate} onChange={(e) => setForm({ ...form, eventDate: e.target.value })} className={inputClass} />
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#b0b0b0] uppercase tracking-[3px] mb-2">Fee (£)</label>
          ...
          <div className="flex justify-end">
            <button type="submit" className="px-7 py-3 border-2 border-[#111] rounded-full text-[13px] font-semibold uppercase tracking-[1.5px] text-[#111] hover:bg-[#3a3a3a] hover:text-white transition">Create Project</button>
          </div>
        </form>
      )}
        ...
        <>
            ...
            <div className="flex gap-4 min-w-[900px]">
              {PIPELINE.map((status) => {
                ...
                return (
                  <div
                    key={status}
                    className="flex-1 min-w-[140px] bg-[#8a8a8a]/50 rounded-lg p-4 border border-[#b0b0b0]/30"
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-xs text-[#b0b0b0]">{statusProjects.length}</span>
                    </div>
                    <div className="space-y-2">
                      {statusProjects.map((p) => (
                        <div
                          ...
                          className="bg-white rounded-lg p-3 border border-[#b0b0b0]/30 hover:border-[#7a7a7a] transition-shadow group cursor-move"
                        >
                          <Link href={`/admin/projects/${p.id}`} className="block">
                            <div className="flex items-start justify-between">
                              <p className="text-sm font-semibold text-[#7a7a7a] leading-tight">{p.title}</p>
                              <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); deleteProject(p.id) }} className="text-xs text-[#b0b0b0] hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity ml-2 flex-shrink-0">
                                <Trash2 size={14} />
                              </button>
                            </div>
                      ...
                      {statusProjects.length === 0 && <p className="text-xs text-[#b0b0b0] italic py-4 text-center">Drop here</p>}
                    </div>
                  </div>
                )
              })}
          ...
          <div className="md:hidden space-y-6">
            {PIPELINE.map((status) => {
              ...
              return (
                <div key={status} className="bg-white border border-[#b0b0b0]/30 p-4">
                  <p className="text-xs font-bold text-[#111] uppercase tracking-[3px] mb-3">{PIPELINE_LABELS[status]} <span className="text-[#b0b0b0] font-normal">({statusProjects.length})</span></p>
                  <div className="space-y-2">
                    {statusProjects.map((p) => (
                      <div key={p.id} className="bg-[#8a8a8a]/50 rounded-lg p-3 border border-[#b0b0b0]/30">
                        <Link href={`/admin/projects/${p.id}`} className="block">
                          <p className="text-sm font-semibold text-[#7a7a7a] leading-tight">{p.title}</p>
                          <p className="text-xs text-[#a0a0a0] mt-0.5">{p.venue || 'No venue'}</p>
                          {p.fee && <p className="text-xs font-semibold text-[#91715c] mt-1">£{Number(p.fee).toLocaleString()}</p>}
                        </Link>
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#b0b0b0]/30">
                          <select
                            value={p.status}
                          ...
                          <button onClick={() => deleteProject(p.id)} className="text-xs text-[#b0b0b0] hover:text-red-500">Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {projects.length === 0 && <p className="text-[#b0b0b0] text-center py-8">No projects yet. Create your first project above.</p>}
          </div>
        </>
      )}
    </div>
