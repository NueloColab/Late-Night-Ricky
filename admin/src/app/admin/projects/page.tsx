...
const PIPELINE_STYLES: Record<string, { bg: string; text: string; border: string }> = {
  inquiry: { bg: 'bg-[#f8f7f6]', text: 'text-[#5B7A8E]', border: 'border-[#e5e5e5]' },
  quoted: { bg: 'bg-[#1B3A4C]/10', text: 'text-[#1B3A4C]', border: 'border-[#1B3A4C]/30' },
  approved: { bg: 'bg-[#6B8FAB]/10', text: 'text-[#6B8FAB]', border: 'border-[#6B8FAB]/30' },
  'in-progress': { bg: 'bg-[#1B3A4C]/10', text: 'text-[#1B3A4C]', border: 'border-[#1B3A4C]/30' },
  completed: { bg: 'bg-[#6B8FAB]/10', text: 'text-[#6B8FAB]', border: 'border-[#6B8FAB]/30' },
...
const inputClass = 'w-full px-4 py-2.5 border border-[#A3B5C4]/30 bg-white border-[#A3B5C4]/30 rounded-md text-gray-900 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-[#1B3A4C] focus:border-transparent transition-all duration-200'
    ...
    <div>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-3">
          ...
          <p className="text-xs uppercase tracking-widest text-[#1B3A4C] font-medium">Project Management</p>
        </div>
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="font-serif text-3xl md:text-4xl font-semibold text-[#111] tracking-tight">Projects</h1>
            <p className="text-sm text-[#5B7A8E] mt-1">Pipeline tracker</p>
          </div>
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="border border-[#A3B5C4]/30 bg-white border-[#A3B5C4]/30 p-5">
          ...
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5B7A8E] font-medium">Total Projects</p>
        </div>
        <div className="border border-[#A3B5C4]/30 bg-white border-[#A3B5C4]/30 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="p-2 bg-[#1B3A4C] text-white rounded-lg"><Clock size={16} /></div>
          </div>
          <p className="text-2xl font-serif font-semibold text-[#1B3A4C]">{loading ? '–' : stats.inquiry}</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5B7A8E] font-medium">Inquiries</p>
        </div>
        <div className="border border-[#A3B5C4]/30 bg-white border-[#A3B5C4]/30 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="p-2 bg-[#6B8FAB] text-white rounded-lg"><TrendingUp size={16} /></div>
          </div>
          <p className="text-2xl font-serif font-semibold text-[#6B8FAB]">{loading ? '–' : stats.inProgress}</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5B7A8E] font-medium">In Progress</p>
        </div>
        <div className="border border-[#A3B5C4]/30 bg-white border-[#A3B5C4]/30 p-5">
          <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#5B7A8E] font-medium">Pipeline Value</p>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={saveProject} className={`${cardClass} mb-8 space-y-5`}>
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
            <h3 className="text-lg font-serif font-light text-[#111]">New Project</h3>
          </div>
          ...
          <div className="flex justify-end">
            <button type="submit" className="px-6 py-3 bg-[#1B3A4C] text-white text-sm font-medium rounded-md hover:opacity-90 transition-opacity">Create Project</button>
          </div>
        </form>
      )}
        ...
        <>
            ...
            <div className="flex gap-4 min-w-[900px]">
              {PIPELINE.map((status) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-xs font-bold text-[#111] uppercase tracking-widest">{PIPELINE_LABELS[status]}</h3>
                    ...
                    <div className="space-y-2">
                      {statusProjects.map((p) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <p className="text-sm font-semibold text-[#111] leading-tight">{p.title}</p>
                            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); deleteProject(p.id) }} className="text-xs text-[#6B8FAB] hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity ml-2 flex-shrink-0">
                              <Trash2 size={14} />
                            </button>
                          </div>
                          <p className="text-xs text-[#5B7A8E] mt-0.5">{p.venue || 'No venue'}</p>
                          {p.fee && <p className="text-xs font-semibold text-[#6B8FAB] mt-1">£{Number(p.fee).toLocaleString()}</p>}
                      ...
                      {statusProjects.length === 0 && <p className="text-xs text-[#6B8FAB] italic py-2 text-center">Drop here</p>}
                    </div>
                  </div>
                )
              })}
          ...
          <div className="md:hidden space-y-6">
            {PIPELINE.map((status) => {
              ...
              return (
                <div key={status} className="border border-[#A3B5C4]/30 bg-white border-gray-100 rounded-lg p-4">
                  <h3 className="text-xs font-bold text-[#111] uppercase tracking-widest mb-3">{PIPELINE_LABELS[status]} <span className="text-[#6B8FAB] font-normal">({statusProjects.length})</span></h3>
                  <div className="space-y-2">
                    {statusProjects.map((p) => (
                      <div key={p.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100">
                          ...
                          <button onClick={() => deleteProject(p.id)} className="text-xs text-[#6B8FAB] hover:text-red-500">Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
