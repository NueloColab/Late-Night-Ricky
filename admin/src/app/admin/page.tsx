    ...
    return (
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="bg-white border border-[#A3B5C4]/30 p-6 animate-pulse">
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        ...
        <Link
          href="/admin/invoices"
          className="bg-[#0f1923] p-6 text-white hover:bg-[#1B3A4C] transition group"
        >
          <div className="flex items-center justify-between mb-4">
            <div className="p-2 bg-white/10">
              <TrendingUp size={18} className="text-[#8FA8BE]" />
            </div>
            <ArrowRight size={16} className="text-[#8FA8BE] group-hover:text-white transition" />
          </div>
          <p className="text-[clamp(24px,3vw,36px)] font-black leading-none mb-2">{formatCurrency(s.revenue)}</p>
          <p className="text-xs uppercase tracking-widest text-[#8FA8BE]">Total Revenue</p>
        </Link>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <ArrowRight size={16} className="text-[#A3B5C4] group-hover:text-[#1B3A4C] transition" />
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-[#E3E8ED] p-6">
          ...
          {s.recentEnquiries.length === 0 ? (
            <p className="text-sm text-[#A3B5C4] py-4">No enquiries yet.</p>
          ) : (
            <div className="space-y-3">
              {s.recentEnquiries.map((e) => (
                ...
                >
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${e.status === 'new' ? 'bg-[#1B3A4C]' : 'bg-[#A3B5C4]'}`} />
                  ...
                  <span className="text-xs text-[#A3B5C4] flex-shrink-0">{formatDate(e.createdAt)}</span>
                </Link>
              ))}
            </div>
          )}
              ...
              <tbody className="divide-y divide-[#E3E8ED]">
                {s.recentQuotes.map((q) => (
                  <tr key={q.id} className="hover:bg-[#F8FAFB]">
                    ...
                    <td className="px-4 py-3 text-[#5B7A8E]">{q.clientName || '—'}</td>
