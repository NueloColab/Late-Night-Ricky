        ...
        <div className="lg:col-span-1">
          <div className="bg-white border-2 border-[#111] p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={startCreate} className="text-xs font-semibold tracking-[1px] uppercase px-4 py-2 bg-[#3a3a3a] text-white hover:bg-[#7a7a7a] transition">
                + New
              </button>
            </div>
            {loading ? (
              ...
              <p className="text-sm text-[#b0b0b0]">No show pages yet.</p>
            ) : (
                ...
                {showPages.map((page) => (
                  <div
                    ...
                    className={`p-4 border-2 cursor-pointer transition ${
                      editing?.id === page.id ? 'border-[#111] bg-[#8a8a8a]' : 'border-[#8a8a8a] hover:border-[#b0b0b0]'
                    }`}
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-semibold text-[15px] text-[#111]">{page.title}</p>
                        <p className="text-xs text-[#b0b0b0] mt-1">/{page.slug}</p>
                      </div>
                      {!page.isActive && (
                        <span className="text-[10px] font-semibold uppercase tracking-[1px] px-2 py-1 bg-[#b0b0b0] text-white rounded-full">Draft</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="lg:col-span-2">
          <div className="bg-white border-2 border-[#111] p-6 md:p-8">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Slug *</label>
                <input
                  type="text"
                  value={form.slug}
                  onChange={(e) => setForm({ ...form, slug: e.target.value })}
                  placeholder="e.g. sidemen"
                  className="w-full border-2 border-[#111] px-4 py-3 text-[15px] bg-transparent outline-none focus:border-[#7a7a7a] transition"
                />
                <p className="text-xs text-[#b0b0b0] mt-1">Used in URL: /show-{'{slug}'}</p>
              </div>

              <div>
                <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Title *</label>
                <input
                  type="text"
                  value={form.title}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Venue *</label>
                  <input
                    type="text"
                    value={form.venue}
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Location *</label>
                  <input
                    type="text"
                    value={form.location}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Season *</label>
                  <input
                    type="text"
                    value={form.season}
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Set Length</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Hero Image URL</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-[1.5px] text-[#b0b0b0] mb-2">Description</label>
                <textarea
                  value={form.description}
                  onChange={(e) => setForm({ ...form, description: e.target.value })}
                  rows={4}
                  className="w-full border-2 border-[#111] px-4 py-3 text-[15px] bg-transparent outline-none focus:border-[#7a7a7a] transition resize-y"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-8">
              <button
                onClick={save}
                disabled={saving}
                className="flex-1 py-3.5 bg-[#3a3a3a] text-white text-sm font-semibold uppercase tracking-[2px] hover:bg-[#7a7a7a] transition disabled:opacity-50"
              >
