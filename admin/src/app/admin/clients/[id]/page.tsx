  ...
  if (loading) {
    return (
      <div className="max-w-6xl mx-auto py-12 text-center">
        ...
        <p className="text-[#b0b0b0] text-sm">Loading client...</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-white border border-[#8a8a8a] p-6 text-center">
          <p className="text-[clamp(28px,4vw,42px)] font-black text-[#111] leading-none tracking-[-1px]">
            {quotes.length}
          </p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">Quotes</p>
        </div>
        <div className="bg-white border border-[#8a8a8a] p-6 text-center">
          <p className="text-[clamp(28px,4vw,42px)] font-black text-[#111] leading-none tracking-[-1px]">
            {invoices.length}
          </p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">Invoices</p>
        </div>
        <div className="bg-white border border-[#8a8a8a] p-6 text-center">
          <p className="text-[clamp(28px,4vw,42px)] font-black text-[#111] leading-none tracking-[-1px]">
            {client.totalRevenue ? formatCurrency(client.totalRevenue) : '—'}
          </p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#b0b0b0] font-medium mt-2">Revenue</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-white border border-[#8a8a8a] text-[#111] text-sm focus:outline-none focus:border-[#7a7a7a]"
              />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2.5 bg-white border border-[#8a8a8a] text-[#111] text-sm focus:outline-none focus:border-[#7a7a7a] resize-none"
              />
            </div>
          </div>
        ) : (
      ...
      <div className="bg-white border border-[#8a8a8a] overflow-hidden">
        <div className="p-6 border-b border-[#8a8a8a]">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-2 text-xs px-2.5 py-1 rounded-full bg-[#7a7a7a1a] text-[#7a7a7a] font-medium">
              {quotes.length}
            </span>
          </div>
        </div>
        <div className="overflow-x-auto">
          {quotes.length === 0 ? (
            <p className="p-6 text-sm text-[#b0b0b0]">No quotes yet.</p>
          ) : (
                ...
                {quotes.map((q) => (
                  <tr key={q.id} className="hover:bg-[#F8FAFB] transition-colors">
                    <td className="px-6 py-4">
                      <Link href={`/admin/quotes/${q.id}`} className="text-sm font-medium text-[#7a7a7a] hover:underline">
      ...
      <div className="bg-white border border-[#8a8a8a] overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {invoices.length === 0 ? (
            <p className="p-6 text-sm text-[#b0b0b0]">No invoices yet.</p>
          ) : (
            <table className="w-full">
              <thead className="bg-[#F8FAFB] border-b border-[#8a8a8a]">
                <tr>
              ...
              <tbody className="divide-y divide-[#8a8a8a]">
                {invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-[#F8FAFB] transition-colors">
                    <td className="px-6 py-4">
                      <Link href={`/admin/invoices/${inv.id}`} className="text-sm font-medium text-[#7a7a7a] hover:underline">
