        ...
        <div className="max-w-[1200px] mx-auto px-8 py-16 grid md:grid-cols-2 gap-10">
          {videos.map((video) => (
            <div key={video.id} className="bg-white border-2 border-[#111] overflow-hidden hover:-translate-y-1.5 transition duration-400">
              <div className="relative pb-[56.25%] bg-[#0f0518]">
                <video
                  src={video.src}
                  poster={video.poster}
                  playsInline
              ...
              <div className="p-8">
                ...
                <p className="text-xs text-[#8b7ab4] uppercase tracking-[1.5px] mt-4 font-semibold">{video.year ? `Dec ${video.year}` : ''}</p>
              </div>
            </div>
          ))}
          ...
          {cards.map((card) => (
            <div key={card.id} className="bg-white border-2 border-[#111] overflow-hidden hover:-translate-y-1.5 transition duration-400">
              ...
              <div className="p-8">
                ...
                {card.subtitle && <p className="text-xs text-[#8b7ab4] uppercase tracking-[1.5px] mb-2 font-semibold">{card.subtitle}</p>}
          ...
          {videos.length === 1 && cards.length === 0 && (
            <div className="bg-[#1a0a2e] border-2 border-[#111] flex flex-col items-center justify-center min-h-[300px] p-10 text-center">
