...
export async function generateMetadata() {
  const show = await getShowPage('abu-dhabi');
  return {
    title: show?.title ? `${show.title} — Late Night Ricky` : 'Abu Dhabi Grand Prix — Late Night Ricky',
    description: show?.description || 'Major festival appearance under the desert stars.',
  };
}

export default async function ShowAbuDhabiPage() {
  const show = await getShowPage('abu-dhabi');

  const title = show?.title || 'Abu Dhabi Grand Prix';
  const venue = show?.venue || 'Skyline Festival';
  const location = show?.location || 'Abu Dhabi';
  const season = show?.season || 'Autumn / Winter 2024';
  const setLength = show?.setLength || '4 Hour Set';
  const heroImage = show?.heroImage || '/assets/press-bg2.jpg';
  const description = show?.description || 'Major festival appearance under the desert stars. The Abu Dhabi Grand Prix closing party brought together racing royalty and music lovers for an unforgettable night.';
          ...
          <div className="grid md:grid-cols-2 min-h-[600px]">
            ...
            <div className="p-16 md:p-20 flex flex-col justify-center max-w-[640px]">
              ...
              <div className="flex gap-3 flex-wrap mt-4">
                ...
                <span className="text-[11px] font-semibold tracking-[1.5px] uppercase px-6 py-2.5 border-2 border-[#b0b0b0] rounded-full text-[#7a7a7a]">5,000+</span>
                <span className="text-[11px] font-semibold tracking-[1.5px] uppercase px-6 py-2.5 border-2 border-[#b0b0b0] rounded-full text-[#7a7a7a]">EDM / Hip-Hop</span>
              </div>
            </div>
          </div>
        </section>
